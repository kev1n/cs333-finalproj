...
ggplot(districts, aes(x = per_pupil_spending, y = avg_math_score)) +
  geom_point(alpha = 0.45, size = 2.2, color = report_blue) +
  geom_smooth(method = "lm", color = ref_grey, linewidth = 0.9, se = FALSE) +
  scale_x_continuous(
...
ggplot(funding_long, aes(x = need_group, y = spending, fill = source)) +
  ...
  scale_fill_manual(values = c("Federal" = report_orange, "State & Local" = report_blue),
                    name = "Funding source") +
  labs(
...
group_colors = c(
  "Economically Disadvantaged"     = report_red,
  "English Language Learner"       = report_orange,
  "Students with Disabilities"     = report_blue,
  "Not Economically Disadvantaged" = report_green
)
